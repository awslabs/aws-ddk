...
export interface AppFlowIngestionStageProps extends StateMachineStageProps {
    ...
    readonly tasks?: appflow.CfnFlow.TaskProperty[];
}
...
export class AppFlowIngestionStage extends StateMachineStage {
    ...
    constructor(scope: Construct, id: string, props: AppFlowIngestionStageProps) {
      ...
      const { flowName, flowExecutionStatusCheckPeriod, destinationFlowConfig, sourceFlowConfig, tasks } = props;
      
      if (!flowName) {
        // Check required props for CfnFlow create and except if not provided
        if (!destinationFlowConfig || !sourceFlowConfig || !tasks) {
        ...
        const flow = new appflow.CfnFlow(this, `${id}-flow`, {
            ...
            tasks: tasks,
            triggerConfig: appflow.CfnFlow.TriggerConfigProperty = {
      ...
      const thing = new appflow.CfnFlow.
      ...
      const flowObjectExecutionStatusWait = new sfn.Wait(
        this,
      ...
      const definition = this.flowObject.next(
          flowObjectExecutionStatusWait
        ).next(
          flowExecutionRecords
        ).next(
          flowObjectExecutionStatus.when(
            sfn.Condition.stringEquals("$.FlowExecutionStatus", "Successful"),
    ...
    private createCheckFlowExecutionTask(id: string): tasks.LambdaInvoke {
        const statusLambdaRole = new iam.Role(
            this,
            `{id}-flow-execution-status-lambda-role`,
            {
        ...
        const statusLambda = new lambda.Function(
            this,
            `${id}-flow-execution-status-lambda`,
            {
